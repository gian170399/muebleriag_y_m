<?php
    ...
    $statement = $conexion->prepare("SELECT * FROM productos WHERE categoria = 'departamento' ");
    $statement->execute();
    $resultado_depa = $statement->fetchAll();
    if (empty($resultado_depa)) {
        $titulo = 'No hay articulos disponibles en "Muebles para departamento"' ;
    } else {
        $titulo = 'Muebles para departamento';
    }
    ...
    $statement = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM productos WHERE categoria = 'departamento' ORDER BY id DESC LIMIT $inicio, $postPorPagina" );
